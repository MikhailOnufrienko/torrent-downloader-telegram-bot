...
from app.entities.user.service import UserContentService, UserService, UserTorrentService, user_content_service, user_service, user_torrent_service
        ...
        while proceed:
            ...
            for torrent in torrents_to_watch:
                ...
                for assoc in user_torrent_associations:
                    ...
                    if user.is_blocked and not user.is_unblocking_message_sent:  # type: ignore
                        await bot_instance.send_message_to_get_acquainted(user.tg_id)
                        user = self._user_svc.set_user_is_unblocking_message_sent(user.id)
                        continue
                    ...
                    if len(user_torrent_associations) < 2:
                        ...
                        # TODO Contents буду сохранять, но надо установить save_path = None, is_deleted = True, ready = False.
                        proceed = False
